...
export class UserService {
  ...
  async updateUserWhitelist(id: string, suimailNs: string): Promise<void> {
    ...
    if (suimailNs === user.suimailNs) throw new BadRequestError('Cannot whitelist self');
  ...
  async updateUserBlacklist(id: string, suimailNs: string): Promise<void> {
    ...
    if (suimailNs === user.suimailNs) throw new BadRequestError('Cannot blacklist self');
  ...
  async getAddressListFeatures(recipientSuimailNs: string, senderId: string) {
    ...
    if (recipientUserWhitelist.some(suimailNs => suimailNs === senderSuimailNs)) {
      senderIsWhitelisted = true;
    }

    if (recipientUserBlacklist.some(suimailNs => suimailNs === senderSuimailNs)) {
