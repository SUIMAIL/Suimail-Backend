...
type BaseMailFields = Pick<IMail, 'id' | 'subject' | 'body' | 'createdAt' | 'digest'>;
...
export class MailResponseDto {
  ...
  static fromEntity(
    mail: IMail,
    attachments?: Attachment[],
  ): Pick<MailListItem, 'id' | 'subject' | 'createdAt' | 'attachmentCount' | 'metadata'> {
    return {
      id: mail.id,
  ...
  static fullFromEntity(mail: MailWithPopulatedFields): MailResponseDto {
    return new MailResponseDto(mail, mail.sender, mail.recipient, mail.attachments, mail.metadata);
  }
}
